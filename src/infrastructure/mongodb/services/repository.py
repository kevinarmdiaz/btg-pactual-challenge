...
class BaseRepository(Generic[ConcreteCollection]):
	...
	async def _update(self, key: str, value: Any, payload: dict[str, Any]) -> ConcreteCollection:
		"""
		...
		try:
			...
			updated_document = await self.schema_class.find_one({key: value}).update(update_query)
			...
			raise DatabaseError(detail=str(e), instance="/database/update")
	
	async def _get(self, key: str, value: Any, error_message: str = "") -> ConcreteCollection:
		"""
	...
	async def count(self) -> int:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/count")
	...
	async def _first(self, by: str = "_id") -> ConcreteCollection:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/first")
	...
	async def _last(self, by: str = "id") -> ConcreteCollection:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/last")
	...
	async def _save(self, payload: dict[str, Any]) -> ConcreteCollection:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/save")
	...
	async def _all(self) -> AsyncGenerator[ConcreteCollection, None]:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/all")
	...
	async def delete(self, id_: UUID4) -> None:
		...
		try:
			...
			raise DatabaseError(detail=str(e), instance="/database/delete")
