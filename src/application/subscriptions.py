...
from src.infrastructure.mongodb import transaction_mongo, UsersCollection, FundsCollection
...
async def suscribe_in_fund(user: UsersCollection, fund: FundsCollection) -> bool:
	...
	async with transaction_mongo():
		return await SubscriptionsRepository().suscribe_in_fund(
			user=user, fund=fund
		)
...
async def cancel_in_fund(user: UsersCollection, fund: FundsCollection) -> bool:
	...
	async with transaction_mongo():
		return await SubscriptionsRepository().cancel_in_fund(
			user=user, fund=fund
		)
